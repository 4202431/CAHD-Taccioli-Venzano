...
class BandMatrix:
    ...
    def compute_band_matrix(self, dim_finale=1000, nome_file_item=None, num_sensibile=1, plot=True, withRCM=True):
        ...
        if original_dataset is not None and nome_file_item is not None:
            ...
            while not any(i):
                if plot:
                    print("Non è stato possibile costruire la matrice. Nuovo tentativo in corso...")
                random_column = np.random.permutation(original_dataset.shape[1])[:dim_finale + num_sensibile]
                square_column_index = list()
                for x in random_column:
                    if x not in lista_sensibili and len(square_column_index) < dim_finale:
                        square_column_index.append(x)
                df_square = original_dataset.iloc[row_index][square_column_index]
                [i, j] = np.where(df_square == 1)

            # larghezza di banda = L + U + 1
            ...
            if withRCM:
                ...
                # concatenazione matrice QID bandizzata e matrice sensitive riordinata
                final_df = pd.concat([df_square_band, df_sensitive_band], axis=1, join='inner')

                # calcolo la larghezza di banda dopo la bandizzazione
